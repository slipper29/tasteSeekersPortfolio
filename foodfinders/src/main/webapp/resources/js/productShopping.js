...
$(document).ready(function(){
	...
	//합계금액, 총합계금액 구하기
	let sum = 0;
	for(let i = 0; i < $(".sitemTotal").length; i++){
		
		let total = $(".sitemTotal").eq(i)
		let price = $(".sitemPrice").eq(i).val()
		let count = $(".sitemCount").eq(i).val()
		
		total.html(price*count + "<span>원</span>")
		sum += (price*1);
	}
	$(".tpPrice").html(sum + "<span>원</span>")
    //합계금액, 총합계금액 구하기 끝
	
	//장바구니 삭제 기능
	$(".sitemBtn").on("click", function(){
		$("#sitemForm").submit();
	})
	
	//주소담기
	$("#sorder_add").on("change", function(){
		$(".address").val($("#sorder_add").val())
	})
